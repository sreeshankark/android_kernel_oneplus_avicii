...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (nd->inode && unlikely(nd->inode->i_state & BIT_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (link->dentry->d_inode && unlikely(link->dentry->d_inode->i_state & BIT_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
		return -ENOENT;
	}
#endif
...
{
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (unlikely(inode->i_state & BIT_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
		return -ENOENT;
	}
#endif
		    ...
		    struct path *path, struct path *root)
{
	...
	restore_nameidata();
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!retval && path->dentry->d_inode && unlikely(path->dentry->d_inode->i_state & BIT_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
		putname(name);
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (child->d_inode && unlikely(child->d_inode->i_state & BIT_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
...
static int may_o_create(const struct path *dir, struct dentry *dentry, umode_t mode)
{
	...
	if (dentry->d_inode && unlikely(dentry->d_inode->i_state & BIT_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
...
skip_orig_flow2:
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (unlikely(dentry->d_inode->i_state & BIT_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
			dput(dentry);
			return -ENOENT;
		}
#endif
	...
	if (dir_inode->i_op->atomic_open) {
		...
		if (unlikely(error == -ENOENT) && create_error)
			error = create_error;
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (!IS_ERR(dentry) && dentry->d_inode && unlikely(dentry->d_inode->i_state & BIT_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
			if (create_error) {
				dput(dentry);
				return create_error;
			}
