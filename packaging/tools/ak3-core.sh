...
dyui_print() {
   until [ ! "$1" ]; do
    local formatted_line=$($BB printf "| %-43s |" "$1")
    ui_print "$formatted_line"
    shift
  done
}
...
backup_partimg() {
    dyui_print " ";
    sleep 0.033333s;
    dyui_print "- Backup current partitions";
    sleep 1s;
    mkdir -p /sdcard/NSK_backups;
    dyui_print "-> Backing up boot partition";
    dd if=/dev/block/bootdevice/by-name/boot$slot of=/sdcard/NSK_backups/boot.img;
    sleep 4s;
    dyui_print "-> Backing up dtbo partition";
    dd if=/dev/block/bootdevice/by-name/dtbo$slot of=/sdcard/NSK_backups/dtbo.img;
    sleep 3s;
    dyui_print "-> Backup completed";
    sleep 1s;
    dyui_print "-> Backup location: /sdcard/NSK_backups";
    sleep 0.033333s;
}
### write_boot (repack ramdisk then build, sign and write image and dtbo)
write_boot() {
  dyui_print "-> Active boot slot: $slot" ;
  sleep 0.033333s;
  dyui_print " ";
  sleep 0.033333s;
  ui_print "|---------------------------------------------|";
