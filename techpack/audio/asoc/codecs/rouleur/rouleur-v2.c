...
static int ext_spk_pa_enable(struct snd_soc_dapm_widget *w,
						...
						int event)
{
	pr_info("%s wname: %s event: %d\n", __func__,
		w->name, event);

	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		gpio_set_value(ext_spk_pa_gpio, SPK_ON);
		pr_info("ext spk enalbe\n");
		break;
	case SND_SOC_DAPM_PRE_PMD:
		gpio_set_value(ext_spk_pa_gpio, !SPK_ON);
		pr_info("ext spk disable\n");
		break;
	}
...
static const struct snd_soc_dapm_widget rouleur_dapm_widgets[] = {
	...
	/*ext spk*/
	SND_SOC_DAPM_SPK("Ext Spk", ext_spk_pa_enable),
...
int is_ext_spk_gpio_support(struct device *dev)
{
	int ret;
	...
	ext_spk_pa_gpio = of_get_named_gpio(dev->of_node, spk_ext_pa, 0);
	if (ext_spk_pa_gpio < 0) {
		pr_err("%s: ext spk missing in dt node\n", __func__);
		goto err;
	} else {
		if (!gpio_is_valid(ext_spk_pa_gpio)) {
			pr_err("%s: Invalid ext spk gpio: %d",
				__func__, ext_spk_pa_gpio);
			goto err;
		}

		ret = gpio_request(ext_spk_pa_gpio, "ext_spk_ctl");
		if (ret) {
			pr_err("%s: Failed to request spk_ctl\n", __func__);
			goto err;
		}
		gpio_direction_output(ext_spk_pa_gpio, !SPK_ON);
	}


	pr_info("ext spk gpio: %d", ext_spk_pa_gpio);
	return 0;
err:
	return -EINVAL;
}
...
static int rouleur_probe(struct platform_device *pdev)
{
	...
	is_ext_spk_gpio_support(&pdev->dev);
