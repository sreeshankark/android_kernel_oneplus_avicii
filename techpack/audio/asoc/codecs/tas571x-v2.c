...
#define pr_fmt(fmt) "tas571x " KBUILD_MODNAME ": " fmt
#include <linux/clk.h>
			     ...
			     unsigned int value)
{
	...
	for(i = 0; i < I2C_RETRY; i++) {
		...
		if (ret == size + 1) {
			//pr_info("%s: write success reg=0x%x val:0x%x\n",__func__, reg, value);
			return 0;
		}
		usleep_range(10*1000, 20*1000);
		pr_err("%s: write fail retry %d  \n",__func__, i);
	}

	if(ret >= 0)
		ret = -EIO;

	pr_err("%s: write fail return ret %d  \n",__func__, ret);
	return ret;
}
	...
	for(i = 0; i < I2C_RETRY; i++) {
		...
		if(ret == ARRAY_SIZE(msgs)) {
			*value = 0;
			...
			//pr_info("%s: read success reg=0x%x val=0x%x\n",__func__, reg, *value);
			return 0;
		}
		usleep_range(10*1000, 20*1000);
		pr_err("%s : read fail retry %d  \n",__func__, i);
	}

	if(ret >= 0)
		ret = -EIO;

	pr_err("%s: read fail return ret %d  \n",__func__, ret);
	return 0;
}
...
struct snd_ctl_elem_value  *ucontrol)
{
	...
	for (i = 0 ; i <= 0xff ; i++ ){
		tas571x_reg_read(i2c, i, &value);
		printk("***tas571x Addr,Reg=(%02x, %02x)\n", i, value);
	}
	return 0;
}
...
static const struct soc_enum tas571x_test_enum[] =
{
...
static const struct snd_kcontrol_new tas5717_controls[] = {
	...
	SOC_ENUM_EXT("Reg Read", tas571x_test_enum[0], get_test_reg, NULL),
