...
#else
	// Handle SUSFS_SUS_SU with possible hook usage (left as original logic allows)
	return ksu_sucompat_user_common(filename_user, "faccessat", false);
#endif
}
...
#else
	// In new SUSFS_SUS_SU mode, `susfs_ksu_handle_stat` can be used for improved handling
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
	return 0; // Let higher-level handle or hook replace this
#else
	return ksu_sucompat_user_common(filename_user, "newfstatat", false);
#endif
#endif
...
struct filename *susfs_ksu_handle_stat(int *dfd, const char __user **filename_user, int *flags)
{
	...
	if (unlikely(IS_ERR(name) || !name->name))
		return name;
